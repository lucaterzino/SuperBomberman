...
public class GameRenderer {
    ...
    private void drawMap(GameMap map) {
        for (int r = 0; r < map.getRows(); r++) {
            for (int c = 0; c < map.getColumns(); c++) {
                ...
                gc.fillRect(x + 5, y + 5, 4, 4); gc.fillRect(x + 40, y + 15, 4, 4);
                gc.fillRect(x + 20, y + 40, 4, 4); gc.fillRect(x + 50, y + 50, 4, 4);
    ...
    private void drawPlayer(Player p) {
        ...
        // Disegno in base alla direzione
        switch (dir) {
            case DOWN: // Fronte
                drawPlayerFront(x, y, s);
                break;
            case UP: // Spalle
                drawPlayerBack(x, y, s);
                break;
            case LEFT: // Sinistra
                drawPlayerSide(x, y, s, true);
                break;
            case RIGHT: // Destra
                drawPlayerSide(x, y, s, false);
                break;
        }
    }
    ...
    private void drawPlayerFront(double x, double y, double s) {
        ...
        gc.fillOval(x + 6, y + 26, 10, 10); // Mano SX
        gc.fillOval(x + 32, y + 26, 10, 10); // Mano DX
        gc.fillOval(x + 12, y + 38, 12, 10); // Piede SX
        gc.fillOval(x + 24, y + 38, 12, 10); // Piede DX
        // Antenna
    ...
    private void drawPlayerBack(double x, double y, double s) {
        // Corpo Blu
        gc.setFill(Color.BLUE); gc.fillRect(x + 16, y + 26, 16, 14);
        // Testa Bianca (Tutta bianca, è il retro del casco)
        gc.setFill(Color.WHITE); gc.fillRect(x + 12, y + 4, 24, 24); 
        gc.setStroke(Color.BLACK); gc.setLineWidth(2); gc.strokeRect(x + 12, y + 4, 24, 24);
        // Mani e Piedi (più nascosti)
        gc.setFill(Color.MAGENTA);
        gc.fillOval(x + 8, y + 28, 8, 8); // Mano SX
        gc.fillOval(x + 32, y + 28, 8, 8); // Mano DX
        gc.fillOval(x + 12, y + 38, 12, 10); // Piede SX
        gc.fillOval(x + 24, y + 38, 12, 10); // Piede DX
        // Antenna
        gc.fillRect(x + 22, y - 2, 4, 6);
    ...
    private void drawPlayerSide(double x, double y, double s, boolean isLeft) {
        // Corpo Blu (più stretto vista laterale)
        ...
        // Faccia Rosa (Solo una striscia laterale)
        gc.setFill(Color.PEACHPUFF);
        if (isLeft) gc.fillRect(x + 14, y + 14, 8, 10); // Faccia a sinistra
        else gc.fillRect(x + 26, y + 14, 8, 10); // Faccia a destra
        
        // Occhio (Uno solo)
        gc.setFill(Color.BLACK);
        if (isLeft) gc.fillRect(x + 16, y + 16, 2, 6);
        else gc.fillRect(x + 30, y + 16, 2, 6);
        
        // Mani e Piedi (Sfalsati per simulare il passo)
        gc.setFill(Color.MAGENTA);
        if (isLeft) {
            gc.fillOval(x + 8, y + 30, 10, 10); // Mano davanti
            gc.fillOval(x + 16, y + 38, 14, 10); // Piede
        } else {
            gc.fillOval(x + 30, y + 30, 10, 10); // Mano davanti
            gc.fillOval(x + 18, y + 38, 14, 10); // Piede
        }
    ...
    private void drawEnemy(Enemy e) {
        double x = e.getX(); double y = e.getY(); double s = Enemy.SIZE;
        double cx = x + s/2; double cy = y + s/2;
        gc.setFill(Color.ORANGERED); gc.fillRect(x + 10, y + 4, s - 20, s - 8); 
        gc.fillRect(x + 4, y + 10, s - 8, s - 20); 
        gc.setFill(Color.ORANGE); gc.fillRect(x + 10, y + 10, 10, 10);
        gc.setFill(Color.BLACK); gc.fillRect(cx - 10, cy - 4, 4, 8); gc.fillRect(cx + 6, cy - 4, 4, 8);  
        gc.setFill(Color.WHITE); gc.fillRect(cx - 10, cy - 4, 2, 2); gc.fillRect(cx + 6, cy - 4, 2, 2);
        gc.setStroke(Color.DARKRED); gc.setLineWidth(2); gc.strokeLine(cx - 6, cy + 10, cx + 6, cy + 10);
    }
    ...
    private void drawBomb(Bomb b) {
        ...
        gc.setFill(b.isRemote() ? Color.DARKRED : Color.BLACK); gc.fillOval(x + offset, y + offset, r, r);
        gc.setFill(Color.WHITE); gc.fillOval(x + offset + r*0.25, y + offset + r*0.25, r*0.2, r*0.2);
        gc.setFill(Color.GOLD); gc.fillRect(x + TILE_SIZE/2.0 - 6, y + offset - 4, 12, 6);
        gc.setStroke(Color.WHITE); gc.setLineWidth(3);
        double mx = x + TILE_SIZE/2.0; double my = y + offset - 4;
        if (b.getTimerFrames() % 20 < 10) gc.strokeLine(mx, my, mx, my - 8);
        else gc.strokeLine(mx, my, mx + 4, my - 8);
        if (!b.isRemote()) {
            gc.setFill((b.getTimerFrames() / 5) % 2 == 0 ? Color.RED : Color.YELLOW); gc.fillOval(mx - 3, my - 12, 6, 6);
        }
    }
    
    private void drawExplosion(Explosion e) {
        double x = e.getCol() * TILE_SIZE; double y = e.getRow() * TILE_SIZE;
        double s = TILE_SIZE; double life = (double) e.getTimer() / 30.0;
    ...
    private void drawPowerUp(PowerUp p) {
        ...
        gc.setStroke(Color.WHITE); gc.setLineWidth(3); gc.strokeRect(x, y, s, s);
        switch(p.getType()) {
            case BOMB_UP: gc.setFill(Color.BLACK); gc.fillOval(cx - 8, cy - 6, 16, 16); break;
            case FIRE_UP: gc.setFill(Color.YELLOW); gc.fillOval(cx - 6, cy - 4, 12, 12); gc.setFill(Color.RED); gc.fillOval(cx - 3, cy - 1, 6, 6); break;
            case SPEED_UP: gc.setFill(Color.RED); gc.fillOval(cx - 10, cy, 20, 8); gc.fillRect(cx - 6, cy - 8, 12, 8); gc.setFill(Color.WHITE); gc.fillOval(cx - 8, cy + 6, 4, 4); gc.fillOval(cx + 4, cy + 6, 4, 4); break;
            case REMOTE: gc.setFill(Color.DARKGRAY); gc.fillRect(cx - 6, cy - 8, 12, 16); gc.setFill(Color.RED); gc.fillOval(cx - 2, cy - 4, 4, 4); break;
    ...
    private void drawObjective(Objective o) {
        ...
        gc.setFill(Color.CYAN); gc.fillOval(cx - s/2, cy - s/2, s, s);
    ...
    public void drawHUD(int lives, int score, int enemiesKilled, double timeLeft, List<PowerUpType> activePowerUps) {
        gc.setFill(Color.web("#008000")); gc.fillRect(0, 0, canvasWidth, HUD_HEIGHT);
        gc.setStroke(Color.ORANGE); gc.setLineWidth(4); gc.strokeRect(2, 2, canvasWidth-4, HUD_HEIGHT-4);
        gc.setFont(Font.font("Monospaced", FontWeight.BOLD, 28)); gc.setTextAlign(TextAlignment.LEFT);
        
        double centerY = HUD_HEIGHT / 2; double startX = 40;
        drawMiniPlayerHead(startX, centerY - 15); startX += 45; 
    ...
    private void drawMiniPlayerHead(double x, double y) {
        double size = 30; gc.setFill(Color.WHITE); gc.fillRect(x, y, size, size - 4); 
    ...
    private void drawClockIcon(double x, double y) {
        double size = 30; gc.setFill(Color.WHITE); gc.fillOval(x, y, size, size);
    ...
    private void drawMiniPowerUp(PowerUpType type, double x, double y) {
        double size = 30; gc.setFill(powerUpColors.get(type)); gc.fillRect(x, y, size, size);
    ...
    public void drawPauseMenu(String[] options, int selectedIndex) {
        ...
        gc.setFill(Color.YELLOW); gc.setFont(Font.font("Monospaced", FontWeight.BOLD, 50));
        gc.setTextAlign(TextAlignment.CENTER); gc.fillText("PAUSE", canvasWidth / 2, y + 80);
        gc.setFont(Font.font("Monospaced", FontWeight.BOLD, 30));
        for (int i = 0; i < options.length; i++) {
            double textY = y + 160 + (i * 60);
            if (i == selectedIndex) { gc.setFill(Color.WHITE); gc.fillText(">", canvasWidth / 2 - 100, textY); } 
    ...
    public void drawPauseOverlay() { drawOverlay("PAUSA", Color.LIGHTBLUE); }
